...
def is_inconsistent_row(row):
    ...
    """
    if (not np.any(row[:-1])) and (row[-1]!=0):
        return True
    else:
        return False
...
def elimination_phase(a):
    ...
    for i in range(num_of_rows(a)):
        ...
        while j<num_of_rows(a):
            ...
            j+=1
        if az:
            return True  #end elimination bcs i to end is all zeros
        leftmost = leftmost_nonzero_index(a,i)
        swap_rows(a,i,leftmost[0])
    
        # print("after swap " + str(i)) # DBG
        # print(a) # DBG
        
        k=i+1
        while k<num_of_rows(a):
            zero_in_pivot_column(a,k,i,leftmost[1])
            k+=1
            
        # print("after zeroing "+str(i)) # DBG
        # print(a) # DBG
        
        for b in range(num_of_rows(a)):
            if is_inconsistent_row(a[b]): 
                # print("inco") # DBG
                return False
    return True
...
def back_substitution_phase(a):
    ...
    for i in range(num_of_rows(a)):
        if np.any(a[i]):
            scale_to_one_in_pivot_column(a,i,leading_entry_index(a[i]))
            #print("pv scl") # DBG
            #print(a) # DBG
            for j in range(i):
                zero_in_pivot_column(a,j,i,leading_entry_index(a[i]))
